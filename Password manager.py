...
def create_password(pass_key, service, admin_pass):
    print('create_password = ', sha256(admin_pass.encode('utf-8') + service.lower().encode('utf-8') + pass_key.encode('utf-8')).hexdigest()[:15])
...
def get_hex_key(admin_pass, service):
    print('get_hex_key = ',sha256(admin_pass.encode('utf-8') + service.lower().encode('utf-8')).hexdigest())
